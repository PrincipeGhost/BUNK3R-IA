...
from bunk3r_core.repo_indexer import RepoIndexer
from bunk3r_api.github_sync import GitHubSyncService
from bunk3r_backend.models import db, GitHubRepo
...
def ide_chat():
    ...
    try:
        ...
        if active_repo:
            repo_path = Path(current_app.config.get('WORKSPACES_DIR', 'bunk3r_backend/workspaces')) / user_id / 'repos' / active_repo
            if repo_path.exists():
...
def get_workspace():
    ...
    try:
        user_id = get_user_id()
        workspace_path = Path(current_app.config.get('WORKSPACES_DIR', 'bunk3r_backend/workspaces')) / user_id / 'repos'
...
def read_file():
    ...
    try:
        ...
        workspace_path = Path(current_app.config.get('WORKSPACES_DIR', 'bunk3r_backend/workspaces')) / user_id / 'repos' / repo_name
