...
from BUNK3R_IA.core.repo_indexer import RepoIndexer
from BUNK3R_IA.api.github_sync import GitHubSyncService
from BUNK3R_IA.models import db, GitHubRepo
...
def ide_chat():
    ...
    try:
        ...
        if active_repo:
            repo_path = Path(current_app.config.get('WORKSPACES_DIR', 'BUNK3R_IA/workspaces')) / user_id / 'repos' / active_repo
            if repo_path.exists():
...
def get_workspace():
    ...
    try:
        user_id = get_user_id()
        workspace_path = Path(current_app.config.get('WORKSPACES_DIR', 'BUNK3R_IA/workspaces')) / user_id / 'repos'
...
def read_file():
    ...
    try:
        ...
        workspace_path = Path(current_app.config.get('WORKSPACES_DIR', 'BUNK3R_IA/workspaces')) / user_id / 'repos' / repo_name
